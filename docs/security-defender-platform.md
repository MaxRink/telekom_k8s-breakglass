...
The plain SMTP development path sends recipients through the SMTP envelope and omits a `Bcc` MIME header. The plain SMTP RCPT failure message omits recipient addresses and remote reply text, exposing a stable rejection error. Other SMTP errors and the TLS mail-library path can still include server-supplied text. Keep this mode limited to trusted development networks and use TLS for production mail.
