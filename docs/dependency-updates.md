...
After updating Kubernetes or controller-runtime dependencies, run `make generate manifests` with the repository's Go toolchain and controller-gen version. Embedded upstream Kubernetes types can change generated CRD schemas and apply-configuration parser models even when local API declarations have not changed.
