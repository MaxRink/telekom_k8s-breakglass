...
- **Audit events: group name no longer redacted in session lifecycle events**: `emitSessionAuditEvent` and `emitSessionExpiredAuditEvent` previously applied `system.RedactGroupName()` to `RequestContext.EscalationName` and `Details["grantedGroup"]`, producing masked values (e.g. `clu***`) in the audit log. Audit trails require full group names for forensic completeness — only debug/application logs should mask group names. Both fields now carry the unredacted `spec.grantedGroup` value.
