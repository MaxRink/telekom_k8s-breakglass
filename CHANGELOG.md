...
- **Audit: `Service.Emit` and `Service.EmitSync` no longer hold the read lock while calling into the manager**: the manager reference is now snapshotted under the read lock and the lock is released before the actual emit call, preventing the read lock from being held during potentially long synchronous writes and allowing concurrent `Reload` calls to proceed without deadlock risk.
